...
# Run colabfold
colabfold_batch --stop-at-score 85 split_${SLURM_ARRAY_TASK_ID} result_${SLURM_ARRAY_TASK_ID}
# Delete unneeded files
cd "result_${SLURM_ARRAY_TASK_ID}"
rm cite.bibtex config.json *.png *_rank_[2-5]_*
PATH=$PATH:~/.local/bin
python3 /gpfswork/rech/rep/ubu39dm/softwares/palmfold/palmfold.py -p /gpfswork/rech/rep/ubu39dm/softwares/palmfold/pol/ -t 0.3 -d .
cd ..
